...
class PenilaianService
{
    ...
    {
        $this->setAlternatifs(PilihanAlternatif::where('id_user', Auth::user()->id)->get());
        $this->setKriterias(Kriteria::all());

        $this->setBobotAlternatifKriterias(TempBobotAlternatifKriterita::where('id_user', Auth::user()->id)->get());
    }
        ...
        foreach ($alternatifs as $alternatif) {
            foreach ($kriterias as $kriteria) {
                $bobot = $bobots
                    ...
                    ->where('alternatif_id', $alternatif['id_alternatif'])
                    ->first();
                if (!$bobot || !$bobot['nilai']) {
                    ...
                    $this->setAlternatifIdError($alternatif['id_alternatif']);
    ...
    public function setNiceView()
    {
        ...
        foreach ($this->getAlternatifs() as $alternatif) {
            ...
            foreach ($this->getKriterias() as $kriteria) {
                ...
                if (!isset($datas[$alternatif['id_alternatif']])) $datas[$alternatif['id_alternatif']] = array();
                $datas[$alternatif['id_alternatif']][$kriteria['id']] = $view_value;

                if (!isset($datas2[$alternatif['id_alternatif']])) $datas2[$alternatif['id_alternatif']] = array();
                $datas2[$alternatif['id_alternatif']][$kriteria['id']] = $value;
            }
        }
    ...
    public function getAlternatif($id)
    {
        ...
        if (!array_key_exists($id, $datas)) {
            $alternatif = $this->getAlternatifs()->where('id_alternatif', $id)->first();
            $datas[$id] = $alternatif;
        }
